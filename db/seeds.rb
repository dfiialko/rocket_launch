...
# @rocket_data = JSON.parse(@response)["rockets"]
#  unless @rocket_data.nil?
#    @rocket_data["defaultPads"].each do |r|
#
#    end
#  end
  unless @data["defaultPads"].nil?
    @data["defaultPads"].split(',').each do |pad_data|
      Rocket.create(name:@data["name"],wiki:@data["wikiURL"],img:@data["imgURL"],defaultPad:[pad_data])
      Rocket.count
      Rocket.last.name
      ...
      unless @pad_data.nil?
        ...
        @rocket_pad_data = JSON.parse(@rocket_pad_response)['pads']
          unless @rocket_pad_data.nil?
            ...
            Pad.create(name:rocket_pad_data["name"],map:rocket_pad_data["mapURL"],pad_id:rocket_pad_data["id"])
          end
        end
     end
    end
  ...
  #RocketPad.create(rocket: rocket,pad: pad)
end
#end
